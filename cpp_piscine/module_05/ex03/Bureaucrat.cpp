...
/*   Updated: 2022/02/05 17:43:13 by utygett          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat :: executeForm(Form const & form) {
	if(form.getGradeForExecute() >= _grade && form.getSigned()){
		form.execute(*this);
		std :: cout << _name << " executed " << form.getName() << std :: endl;
	} else if (form.getGradeForExecute() >= _grade && form.getSigned() == false)
		std :: cout << _name << " can't execute " << form.getName() << " because form not sign." << std :: endl;
	else
		std :: cout << _name << " can't execute " << form.getName() << " because his grade too low." << std :: endl;
}
